...
try {
  ...
  assert.ok(packageJson.dependencies['express'], 'express must be in dependencies');
  assert.ok(
  ...
  assert.ok(packageJson.dependencies['dotenv'], 'dotenv must be in dependencies');
  assert.ok(
  ...
  assert.ok(envExampleContent.includes('SUPABASE_URL'), 'SUPABASE_URL must be documented');
  assert.ok(
  ...
  assert.ok(fs.existsSync(path.join(backendDir, 'server.js')), 'server.js must exist');
  assert.ok(fs.existsSync(path.join(backendDir, 'routes', 'ai.js')), 'routes/ai.js must exist');
  assert.ok(
  ...
  const aiRouteContent = fs.readFileSync(path.join(backendDir, 'routes', 'ai.js'), 'utf8');
  ...
  assert.ok(aiRouteContent.includes('Inicializar el cliente'), 'Should contain spanish comments');
  assert.ok(
  ...
  // API key leak check
  assert.ok(!aiRouteContent.includes('sk-ant'), 'Should not contain hardcoded Anthropic keys');
